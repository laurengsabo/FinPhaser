...
IncorporateIBDandIQR = function(df) {
    ...
    # 1. Calculate kinship coefficient
    df$kinship = (df$IBD1/4) + (df$IBD2/2)
    
    for(chr in chromosomes) {
        # Construct the file path safely
        # Note: Ensure 'folder' ends with / or use file.path()
        target_file <- paste0(folder, vcf, ".Chr", chr, ".vcf.gz-truffle.ibd.gz")
        
        print(paste("Reading TRUFFLE chromosome", chr))
        
        # FIX: Check if file exists first to avoid the 'if(NA)' crash
        if (!file.exists(target_file)) {
            print(paste("Skipping:", chr, "- File not found at", target_file))
            next
        }
        
        # Double check size to ensure it's not a header-only file
        if (file.size(target_file) < 1000) {
            print(paste("TRUFFLE did not run for", chr, "(too few loci)"))
            next
        }
        
        # 2. Read and format chromosome-specific IBD0 data
        truffle_tmp = fread(target_file, data.table = FALSE)
        
        # Rename IBD0 to be chromosome-specific
        chr_col_name = paste0("IBD0_", chr)
        truffle_tmp[, chr_col_name] = truffle_tmp$IBD0
        
        # Keep only necessary columns for the join
        truffle_tmp = truffle_tmp[, c("ID1", "ID2", chr_col_name)]
        
        # 3. Join with main dataframe
        df = left_join(df, truffle_tmp, by = c("ID1", "ID2"))
        
        # Remove duplicates created by join (if any)
        df = df[!duplicated(paste(df$ID1, df$ID2)), ]
        
        # 4. Clean up the file to save space
